...
async function register() {
    ...
    try {
        ...
        movieAppDB.users.push(userWithId);
        movieAppDB.favorites[userWithId.id] = [];
        saveDB();

        currentUser = userWithId;
        localStorage.setItem('movieAppCurrentUser', JSON.stringify(userWithId));
        showMovieSearch();
...
async function login() {
    ...
    try {
        ...
        const serverUser = Array.isArray(serverUsers) 
            ? serverUsers.find(u => u.email === email && u.password === password)
            : null;

        if (serverUser) {
            if (!movieAppDB.users.some(u => u.id === serverUser.id)) {
                movieAppDB.users.push(serverUser);
                movieAppDB.favorites[serverUser.id] = [];
                saveDB();
            }

            currentUser = serverUser;
            localStorage.setItem('movieAppCurrentUser', JSON.stringify(serverUser));
            showMovieSearch();
            document.getElementById('login-form').reset();
        } else {
...
function displayMovieDetails(movie) {
    ...
    details.querySelector('.favorite-btn').addEventListener('click', function () {
